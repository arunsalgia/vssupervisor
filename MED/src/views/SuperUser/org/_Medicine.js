...
export default function Medicine() {
  ...
  async function updateMedicineArray(myChar = "") {
		...
		let myData = [];
		try {
			var resp = await axios.get(`${process.env.REACT_APP_AXIOS_BASEPATH}/medicine/count/${userCid}`)
			myData = resp.data;
		} catch (e) {
			console.log(e);
		}
	...
	async function fetchMedicinesByAlphabet(ccc) {
		...
		let myMed = [];
		try {
			let myUrl = `${process.env.REACT_APP_AXIOS_BASEPATH}/medicine/alphabetlist/${userCid}/${ccc}`
	...
	async function handleAddEditSelect() {
		...
		if (modalIsOpen == "ADD") {
			try {
				myUrl = `${process.env.REACT_APP_AXIOS_BASEPATH}/medicine/add/${userCid}/${myName}/${myName}/${myName}`;
			...
			try {
				myUrl = `${process.env.REACT_APP_AXIOS_BASEPATH}/medicine/edit/${userCid}/${medicineName}/${myName}/${myName}/${myName}`;
				await axios.get(myUrl);
	...
	async function handleDeleteMedicine(name) {
		...
		try {
			let myUrl = `${process.env.REACT_APP_AXIOS_BASEPATH}/medicine/delete/${userCid}/${name}`
			await axios.get(myUrl);
