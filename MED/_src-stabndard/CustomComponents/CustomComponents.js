...
export function DisplayYesNo(props) {
	...
	return(
		<div align="center">
			{(myTitle != "") && 
				<div>
				<Typography className={gClasses.header}>{myTitle}</Typography>
				<BlankArea />
				</div>
			}
			{(myMessage != "") && 
				<Typography className={gClasses.header}>{myMessage}</Typography>
			}
			<BlankArea />
			{(myButton1 != "") && <Button variant="contained" size="small" color="primary" className={gClasses.submit}
			onClick={() => { props.close(); props.func(myId, "YES"); }}>
			{myButton1}
			</Button>}
			{(myButton2 != "") && <Button variant="contained" size="small" color="secondary" className={gClasses.submit}
			onClick={() => { props.close(); props.func(myId, "NO");}}>
			{myButton2}
			</Button>}
		</div>
	)}
