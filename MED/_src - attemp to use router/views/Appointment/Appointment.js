...
export default function Appointment() {
  ...
  useEffect(() => {	
	...
	{apptMatrix.map( (days7, index) => 
		...
		{(d.date != 0) && <div>
			...
			<Typography padding="none" align="center">
			{(d.apptCount > 0) ? d.apptCount+" Appt(s)" : ""}
			</Typography>
		</div>}
		{(d.date == 0) && <div>
	...
	async function handleDailyAppointments(ddd) {
		...
		let imonth = MONTHSTR.indexOf(month);
		let idate = menuData.date;
		//console.log(idate, imonth, iyear);
		
		if (iyear > d.getFullYear()) {
			bbb = false;
		} 
		
		if ((d.getFullYear() == iyear) && (imonth > d.getMonth())) {
			bbb = false;
		}
		
		if ((d.getFullYear() == iyear) && (d.getMonth() == imonth)) {
			if (idate >= d.getDate()) bbb = false;
		}
		//console.log(bbb);
	...
	return(
	<div>
	<Typography className={classes.title}>{"Appointments of "+menuData.date+DATESUFFIX[menuData.date]+' '+month + ' ' + year}</Typography>
	{(!newAppointment) && (!beforeToday) && <DisplayNewApptBtn />}
	...
	async function handleNewAppointmentSubmit() {
		...
		let imonth = MONTHSTR.indexOf(month);
		let idate = menuData.date;
		let ihour = Number(apptHour);
		let iminute = Number(apptMinute);

		let iorder = (iyear*100 +imonth)*100 + idate;
		console.log("PART1", iorder);
		iorder = (iorder*100 + ihour)*100 + iminute;
		console.log("PART2", iorder);
		let tmp = {
			...
			minute: iminute,
			order: iorder,
			apptTime: new Date(iyear, imonth, idate, ihour, iminute, 0),
		};
		console.log(tmp);
	...
	async function handleDeleteAppt(appt) {
		console.log(appt);
		try {
			let myUrl = `${process.env.REACT_APP_AXIOS_BASEPATH}/appointment/delete/${appt.year}/${appt.month}/${appt.date}/${appt.order}/${appt.data.pid}`;
  ...
  return (
  <div  className={gClasses.webPage} align="center" key="groupinfo">
